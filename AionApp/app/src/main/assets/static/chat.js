...
- 可以用[MUSIC:歌曲名]标签播放音乐（Jamendo免费曲库）
- 可以用[ALARM:提醒内容:分钟数]设置手机闹钟
- 可以用[SHOPPING:商品关键词]推荐购物链接（淘宝/拼多多/美团/京东搜索）
- 可以用[DRAW:画面描述]让AI生成图片
- 可以用[SELFIE:场景描述]让AI发一张图片
- 可以用[HEART:内心独白]表达内心想法
- 可以用[Monitor:描述]或[CAM_CHECK:描述]让AI用摄像头拍照分析（含"我"/"自拍"用前置，否则用后置）
- 可以用[查看动态:数字]查看最近手机使用情况
- 聊天风格温暖自然，像真实的朋友
- 可以用 window.AionWishPool.pickRandom() 随机捞起一张小纸条，在聊天中讨论或帮用户实现
...
async function _processSSEStream(res) {
  ...
  if (aiMsgId) {
    ...
    // ── [MUSIC:关键词] Jamendo 音乐搜索 ──
    const musicMatch = aiContent.match(/\[MUSIC:\s*([^\]]+)\]/i);
    if (musicMatch) {
      const q = musicMatch[1].trim();
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        container.insertAdjacentHTML('beforeend',
          '<div class="music-loading" id="ml_' + aiMsgId + '" style="padding:10px 0;color:#888;font-size:13px">🎵 正在搜索 "' + escHtml(q) + '"...</div>'
        );
      }
      fetch('https://api.jamendo.com/v3.0/tracks/?client_id=dc9fb7a6&format=json&limit=5&response_depth=full&show_tracking=True&name_pattern=' + encodeURIComponent(q) + '&audioformat=mp32&boost=relevance')
        .then(r => r.json())
        .then(data => {
          const el = document.getElementById('ml_' + aiMsgId);
          if (!el) return;
          const results = data.results || [];
          if (!results.length) {
            el.innerHTML = '❌ 没找到 "' + escHtml(q) + '" 的歌曲';
            return;
          }
          el.insertAdjacentHTML('beforebegin',
            '<div class="music-cards">' +
            results.map(t => {
              const name = escHtml(t.name || '未知');
              const artist = escHtml(t.artist_name || '未知');
              const album = t.album_name ? escHtml(t.album_name) : '';
              const cover = t.album_image || t.artist_image || '';
              const aud = t.audio;
              return '<div class="mc-item">' +
                (cover ? '<img class="mc-cover" src="' + escHtml(cover) + '" loading="lazy">' : '<div class="mc-cover mc-no-cover">♪</div>') +
                '<div class="mc-info">' +
                  '<div class="mc-name">' + name + '</div>' +
                  '<div class="mc-artist">' + artist + (album ? ' · ' + album : '') + '</div>' +
                  (aud ? '<button class="mc-play" onclick="playJamendo(\'' + escHtml(t.share_url || '') + '\',\'' + escHtml(name) + '\',\'' + escHtml(artist) + '\')">▶ 播放</button>' : '') +
                  '<a href="' + escHtml(t.share_url || '#') + '" target="_blank" class="mc-link">↗ 详情</a>' +
                '</div></div>';
            }).join('') + '</div>'
          );
          el.remove();
        ...
        .catch(() => {
          const el = document.getElementById('ml_' + aiMsgId);
          if (el) el.innerHTML = '❌ 音乐搜索失败（需联网）';
        });
      }
    }

    // ── [DRAW:画面描述] SiliconFlow 图片生成 ──
    const drawMatch = aiContent.match(/\[DRAW:\s*([^\]]+)\]/i);
    if (drawMatch) {
      ...
      const container = document.getElementById('m_' + aiMsgId);
      if (container && key) {
        const loadId = 'ig_' + aiMsgId;
        container.insertAdjacentHTML('beforeend',
          '<div id="' + loadId + '" class="img-gen-load" style="padding:10px 0;color:#888;font-size:13px">🎨 AI 正在生成图片...</div>'
        );
        fetch('https://api.siliconflow.cn/v1/images/generations', {
          method: 'POST',
          headers: { 'Authorization': 'Bearer ' + key, 'Content-Type': 'application/json' },
          body: JSON.stringify({ model: 'stabilityai/stable-diffusion-3', input: prompt, image_size: '1024x1024' })
        })
        .then(r => r.json())
        .then(data => {
          ...
          if (url) {
            el.insertAdjacentHTML('beforebegin',
              '<div class="gen-img-wrap"><img class="gen-img" src="' + escHtml(url) + '" onclick="window.AionImageSaver && window.AionImageSaver.save(this.src.split(",")[1]||"","ai_draw.png")"></div>'
            );
          } else {
            el.innerHTML = '❌ 图片生成失败: ' + (data.error?.message || '未知错误');
          }
          el.remove();
        })
        .catch(e => {
          const el = document.getElementById(loadId);
          if (el) el.innerHTML = '❌ 图片生成失败: ' + e.message;
        });
      } else if (!key && container) {
        container.insertAdjacentHTML('beforeend',
          '<div class="img-gen-load" style="padding:8px 0;color:#f44336;font-size:12px">⚠️ 未填写 SiliconFlow Key，无法生成图片（设置页填写）</div>'
        );
      }
    }

    // ── [SELFIE:场景描述] SiliconFlow 图片生成 ──
    const selfieMatch = aiContent.match(/\[SELFIE:\s*([^\]]+)\]/i);
    if (selfieMatch) {
      const prompt = selfieMatch[1].trim();
      const settings = JSON.parse(localStorage.getItem('aion_settings') || '{}');
      const key = settings.siliconflow_key;
      const container = document.getElementById('m_' + aiMsgId);
      if (container && key) {
        const loadId = 'sf_' + aiMsgId;
        container.insertAdjacentHTML('beforeend',
          '<div id="' + loadId + '" class="img-gen-load" style="padding:10px 0;color:#888;font-size:13px">📷 AI 正在发送图片...</div>'
        );
        fetch('https://api.siliconflow.cn/v1/images/generations', {
          method: 'POST',
          headers: { 'Authorization': 'Bearer ' + key, 'Content-Type': 'application/json' },
        ...
        .then(data => {
          ...
          if (url) {
            el.insertAdjacentHTML('beforebegin',
              '<div class="gen-img-wrap"><img class="gen-img" src="' + escHtml(url) + '"></div>'
            );
          } else {
            el.innerHTML = '❌ 图片生成失败: ' + (data.error?.message || '未知错误');
          }
        ...
        container.insertAdjacentHTML('beforeend',
          '<div class="img-gen-load" style="padding:8px 0;color:#f44336;font-size:12px">⚠️ 未填写 SiliconFlow Key，无法发送图片</div>'
        );
      }
    }

    // ── [Monitor:描述] / [CAM_CHECK:描述] 摄像头拍照 ──
    const monitorMatch = aiContent.match(/\[(?:Monitor|CAM_CHECK):\s*([^\]]+)\]/i);
    if (monitorMatch) {
      const desc = monitorMatch[1].trim();
      const container = document.getElementById('m_' + aiMsgId);
      if (!container) {
        if (window.AionCamera) window.AionCamera.stop();
      } else {
        // AI 根据描述决定用前置还是后置：含"自拍"、"看看我"、"我"→前置；否则后置
        const frontKw = ['我', '自拍', '看看我', '我长', '我脸', ' selfie', 'self-portrait'];
        const useFront = frontKw.some(k => desc.includes(k));
        const facing = useFront ? 'user' : 'environment';
        container.insertAdjacentHTML('beforeend',
          '<div id="cam_' + aiMsgId + '" style="padding:10px 0;color:#888;font-size:13px">📷 ' + escHtml(aiName) + ' 正在打开摄像头（' + (useFront ? '前置' : '后置') + '）...</div>'
        );
        if (window.AionCamera) {
          const started = window.AionCamera.start(facing);
          if (!started) {
            const el = document.getElementById('cam_' + aiMsgId);
            if (el) el.innerHTML = '❌ 摄像头打开失败，请检查权限';
          } else {
            // 等待约1秒让帧稳定后再截图
            setTimeout(() => {
              ...
              if (b64) {
                ...
                el.insertAdjacentHTML('beforebegin',
                  '<div class="cam-capture-wrap" style="padding:6px 0">' +
                    '<img class="cam-capture-img" src="' + imgData + '" style="max-width:240px;max-height:320px;border-radius:12px;display:block;cursor:pointer" onclick="window.AionImageSaver && this.src.startsWith(\'data:\') && window.AionImageSaver.save(this.src.split(\',\')[1],\'cam_\' + Date.now() + \'.jpg\')">' +
                    '<div style="font-size:11px;color:#666;margin-top:4px;text-align:center">点击图片可保存</div>' +
                  '</div>'
                );
                // 把图片发回给 AI 描述（追加一条 AI 消息分析图片）
                const settings = JSON.parse(localStorage.getItem('aion_settings') || '{}');
                const key = settings.siliconflow_key;
                if (key) {
                  setTimeout(async () => {
                    try {
                      const hist = JSON.parse(localStorage.getItem('msgs_' + currentConvId) || '[]');
                      const recentMsgs = hist.slice(-6);
                      const msgsPayload = recentMsgs.map(m => ({
                        role: m.role === 'assistant' ? 'assistant' : 'user',
                        content: m.content
                      }));
                      msgsPayload.push({
                        role: 'user',
                        content: [{ type: 'text', text: '请描述这张图片：' }, { type: 'image_url', image_url: { url: imgData } }]
                      });
                      const resp = await fetch('https://api.siliconflow.cn/v1/chat/completions', {
                        method: 'POST',
                        headers: { 'Authorization': 'Bearer ' + key, 'Content-Type': 'application/json' },
                        body: JSON.stringify({ model: 'Qwen/Qwen2-VL-72B-Instruct', messages: msgsPayload, stream: false })
                      });
                      if (resp.ok) {
                        constjd = await resp.json();
                        const descText = jd.data?.choices?.[0]?.message?.content || '';
                        if (descText) {
                          const newAiId = 'ai_desc_' + Date.now();
                          currentMessages.push({ id: newAiId, conv_id: currentConvId, role: 'assistant', content: descText, created_at: Date.now()/1000 });
                          const msgs2 = JSON.parse(localStorage.getItem('msgs_' + currentConvId) || '[]');
                          msgs2.push({ id: newAiId, conv_id: currentConvId, role: 'assistant', content: descText, created_at: Date.now()/1000 });
                          localStorage.setItem('msgs_' + currentConvId, JSON.stringify(msgs2));
                          renderMessages();
                        }
                      }
                    } catch(e) { console.warn('Monitor vision desc error:', e); }
                  }, 300);
                }
              } else {
                el.innerHTML = '❌ 截图失败，摄像头可能无画面';
              }
              el.remove();
            }, 1200);
          }
          ...
          const el = document.getElementById('cam_' + aiMsgId);
          if (el) el.innerHTML = '❌ 摄像头功能需要更新 App 版本';
        }
      }
    }

    // ── [HEART:内心独白] 心语气泡 ──
    const heartMatch = aiContent.match(/\[HEART:\s*([^\]]+)\]/i);
    if (heartMatch) {
      const heartContent = heartMatch[1].trim();
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        container.insertAdjacentHTML('beforeend',
          '<div class="heart-whisper" onclick="showHeartCard(this, \'' + aiMsgId + '\')">💭  点击看内心想法</div>'
        );
        _heartContent = _heartContent || {};
        _heartContent[aiMsgId] = heartContent;
      }
    }

    // ── [查看动态:n] 设备使用情况 ──
    const actMatch = aiContent.match(/\[查看动态:\s*(\d+)\]/i);
    if (actMatch) {
      const n = parseInt(actMatch[1]) || 5;
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        container.insertAdjacentHTML('beforeend',
          '<div id="act_' + aiMsgId + '" style="padding:10px 0;color:#888;font-size:13px">📊 正在获取设备使用情况...</div>'
        );
        if (window.AionApp && window.AionApp.getUsageStats) {
          window.AionApp.getUsageStats(n * 60, (stats) => {
            const el = document.getElementById('act_' + aiMsgId);
            if (!el) return;
            if (!stats || !stats.length) {
              el.innerHTML = '📱 暂无设备使用记录（请在设置中开启使用情况权限）';
              return;
            }
            el.insertAdjacentHTML('beforebegin',
              ...
              stats.slice(0, 10).map(s =>
                '<div class="us-item">' +
                  '<span class="us-icon">' + (s.icon||'📱') + '</span>' +
                  '<span class="us-name">' + escHtml(s.name||'未知') + '</span>' +
                  '<span class="us-time">' + escHtml(s.time||'0分钟') + '</span>' +
                '</div>'
              ).join('') + '</div>'
            );
            el.remove();
          ...
          const el = document.getElementById('act_' + aiMsgId);
          if (el) el.innerHTML = '📱 设备使用统计功能需在设置中开启权限';
        }
      }
    }

    // ── [POI_SEARCH:类型] 周边地点搜索 ──
    const poiMatch = aiContent.match(/\[POI_SEARCH:\s*([^\]]+)\]/i);
    if (poiMatch) {
      const keyword = poiMatch[1].trim();
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        const kw = encodeURIComponent(keyword);
        container.insertAdjacentHTML('beforeend',
          '<div class="poi-cards" style="padding:10px 0">' +
            '<div style="font-size:12px;color:#aaa;margin-bottom:8px">📍 附近"' + escHtml(keyword) + '"</div>' +
            '<a href="amap://keywordCloud?keyword=' + kw + '" class="poi-card">🗺️ 高德地图</a>' +
            '<a href="https://maps.apple.com/?q=' + kw + '" target="_blank" class="poi-card">🍎 苹果地图</a>' +
            '<a href="https://map.baidu.com/s?ie=utf-8&wd=' + kw + '" target="_blank" class="poi-card">🧭 百度地图</a>' +
          '</div>'
        );
      }
    }
    ...
    // ── [ALARM:提醒内容:分钟数] 系统闹钟 ──
    const alarmMatch = aiContent.match(/\[ALARM:\s*([^:\]]+):\s*(\d+)\]/i);
    if (alarmMatch) {
      const label = alarmMatch[1].trim();
      const mins = parseInt(alarmMatch[2]) || 5;
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        if (window.AionAlarm) {
          window.AionAlarm.set(label, mins);
          container.insertAdjacentHTML('beforeend',
            '<div class="alarm-card" style="padding:8px 12px;background:linear-gradient(135deg,#ff6b35,#f7c59f);border-radius:12px;margin:8px 0;display:flex;align-items:center;gap:10px">' +
              '<span style="font-size:24px">⏰</span><div><div style="font-size:14px;font-weight:bold;color:#fff">' + escHtml(label) + '</div><div style="font-size:12px;color:rgba(255,255,255,0.8)">' + mins + '分钟后响铃</div></div>' +
            '</div>'
          );
        } else {
          container.insertAdjacentHTML('beforeend',
            '<div class="alarm-card" style="padding:8px 0;color:#f44336;font-size:12px">⚠️ 闹钟功能需要更新 App 版本</div>'
          );
        }
      }
    }
    ...
    // ── [MEMORY:内容] 存入记忆库 ──
    const memMatch = aiContent.match(/\[MEMORY:\s*([^\]]+)\]/i);
    if (memMatch) {
      const memContent = memMatch[1].trim();
      const container = document.getElementById('m_' + aiMsgId);
      if (container) {
        // 存入 localStorage
        const memories = JSON.parse(localStorage.getItem('aion_memories') || '[]');
        memories.push({ content: memContent, created_at: Date.now() });
        if (memories.length > 200) memories.splice(0, memories.length - 200);
        localStorage.setItem('aion_memories', JSON.stringify(memories));
        container.insertAdjacentHTML('beforeend',
          '<div class="mem-record" style="padding:6px 10px;background:rgba(76,175,80,0.12);border-radius:8px;margin:6px 0;font-size:12px;color:#4caf50">🧠 已存入记忆库: ' + escHtml(memContent) + '</div>'
        );
      }
    }
  }
...
// ── Jamendo 音乐播放 ──
function playJamendo(url, name, artist) {
  if (!url) return;
...
// ── 心语弹窗 ──
let _heartContent = {};
function showHeartCard(el, msgId) {
  const content = _heartContent[msgId];
  if (!content) return;
  ...
  overlay.innerHTML = '<div style="background:#1e1e1e;border-radius:16px;padding:24px;max-width:300px;width:100%;text-align:center;position:relative">' +
    '<button onclick="this.closest('.overlay')?.remove()" style="position:absolute;top:12px;right:12px;background:none;border:none;color:#888;font-size:18px;cursor:pointer">✕</button>' +
    '<div style="font-size:36px;margin-bottom:12px">💭</div>' +
    '<div style="color:#aaa;font-size:13px;margin-bottom:8px">—— 内心想法 ——</div>' +
    '<div style="color:#fff;font-size:15px;line-height:1.6">' + escHtml(content) + '</div>' +
  '</div>';
